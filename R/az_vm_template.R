...
public=list(
    ...
    initialize=function(token, subscription, resource_group, name, location, os=c("Windows", "Ubuntu"),
                        ...
                        extensions=NULL, template, parameters, ..., wait=TRUE)
    {
        ...
        existing_vm <- missing(location) && missing(size) && missing(os) &&
                       ...
                       missing(extensions) && missing(template) && missing(parameters) &&
                       is_empty(list(...))
        ...
        if(!existing_vm) # we want to deploy
        {
            ...
            # find bundled template given input args
            if(missing(template))
                template <- private$get_dsvm_template(os, userauth_type, extensions)
...
private=list(
    ...
    get_dsvm_template=function(os, userauth_type, extensions)
    {
    ...
    make_dsvm_param_list=function(name, username, userauth_type, passkey, size, extensions, template)
    {
